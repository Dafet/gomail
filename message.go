...
func (m *Message) appendBytes(list []*file, name string, body []byte, settings []FileSetting) []*file {
	f := &file{
		Name:   filepath.Base(name),
		Header: make(map[string][]string),
		CopyFunc: func(w io.Writer) error {
			h := bytes.NewReader(body)
			if _, err := io.Copy(w, h); err != nil {
				return err
			}
			return nil
...
// Attach attaches the files to the email.
func (m *Message) AttachBytes(filename string, body []byte, settings ...FileSetting) {
	m.attachments = m.appendBytes(m.attachments, filename, body, settings)
}
